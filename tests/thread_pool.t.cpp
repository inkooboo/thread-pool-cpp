    ...
    doTest("process job with exception", []()
        {
            ...
            {
                ASSERT(r.get() == 42 &&
                       !"should not be called, exception expected");
            }
    ...
    doTest("multiple compilation units", []()
        {
            ...
            std::future<std::tuple<size_t, size_t, size_t, size_t>> r =
                pool.process([]()
                    {
                        return std::make_tuple(
                            Worker::getWorkerIdForCurrentThread(),
                            *detail::thread_id(), getWorkerIdForCurrentThread(),
